使用说明书 - 个人博客项目
1. 项目概述
这是一个基于 Jekyll 的静态博客网站，托管于 GitHub Pages（https://zor666.me），支持中英文内容，适合分享随笔、技术笔记、旅行日志等。项目通过 GitHub Actions 自动构建和部署，使用自定义插件生成标签页面，并支持动态内容加载和多语言切换。
主要功能
...
克隆项目到本地：git clone https://github.com/wesson2775/wesson2775.github.io.git
cd wesson2775.github.io


确保 .gitignore 包含 _site 和 .jekyll-cache。

3.3 安装依赖

安装 Ruby 和 Bundler：
macOS/Linux：sudo apt-get install ruby-full build-essential zlib1g-dev  # Ubuntu
brew install ruby  # macOS
gem install bundler


Windows：使用 RubyInstaller 安装 Ruby 和 Bundler。
...
启动 Jekyll 本地服务器：bundle exec jekyll serve --livereload


打开浏览器，访问 http://localhost:4000。
修改文件后，页面会自动刷新（--livereload）。
...
中英文标签翻译在 utils.js 的 tagTranslations 中定义：tagTranslations: {
...
在 _fragments/zh/ 或 _fragments/en/ 创建 Markdown 文件，例如 _fragments/zh/2025-01-01-新年随笔.md：---
title: 新年随笔
date: 2025-01-01
lang: zh
category: fragments
...
保存后，Jekyll 自动生成随笔页面（例如 /fragments/zh/新年随笔/）。

6.2 添加标签

在随笔的 tags 字段添加新标签，例如 tags: [新标签]。
更新 utils.js 的 tagTranslations：tagTranslations: {
    zh: { '新标签': '新标签' },
    en: { '新标签': 'New Tag' }
}
...
编辑 assets/main.scss 调整样式，例如：.fragment-content {
    margin: 1em 0;
    line-height: 1.6;
}
...
访问 https://zor666.me 查看更新（可能需等待几分钟）。
...
更新导航（在 _layouts/default.html 或 utils.js）。
