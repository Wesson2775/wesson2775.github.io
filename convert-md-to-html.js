...
const modelContent = fs.readFileSync("model.html", "utf8");
...
for (const file of files) {
  try {
    ...
    const finalHtml = modelContent.replace(
      "<div class=\"blog-content\">121232</div>",
  ...
  const scriptsContent = `
    document.addEventListener("DOMContentLoaded", async () => {
      ...
      for (const post of posts) {
        try {
          const response = await fetch(postsPath + post);
          if (!response.ok) throw new Error("HTTP错误! 状态: " + response.status);

          const text = await response.text();
          const parser = new DOMParser();
          const doc = parser.parseFromString(text, "text/html");
          const blogContent = doc.querySelector(".blog-content")?.textContent;
